...
export async function connectToDatabase(filename: string) {
  ...
  // Register cleanup handlers - but don't call closeDatabase yet!
  // SIGINT is for Ctrl+C
  process.on("SIGINT", () => {
    console.log("\nSQLite connection closed due to app termination");
    if (rawDatabase) {
      rawDatabase.close();
    }
    process.exit(0);
  ...
  // SIGTERM is for kill signals
  process.on("SIGTERM", () => {
    console.log("SQLite connection closed due to termination signal");
    if (rawDatabase) {
      rawDatabase.close();
    }
    process.exit(0);
